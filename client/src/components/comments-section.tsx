...
export function CommentsSection({ entityType, entityId, forumId, initialCommentCount = 0 }: CommentsSectionProps) {
  const { user } = useAuth();
  const { toast } = useToast();
  const isMobile = useIsMobile();
  ...
  const commentCount = comments ? comments.reduce((count, comment) => {
