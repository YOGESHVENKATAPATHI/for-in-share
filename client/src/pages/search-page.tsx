...
export default function SearchPage() {
  ...
  const { data, isLoading } = useQuery<SearchResults>({
    ...
    onSuccess: (data) => {
      if (data?.files) {
        console.log("[SearchPage] Fetched files from backend:");
        data.files.forEach((file) => {
          console.log(`ID: ${file.id}, Name: ${file.fileName}, Tags: ${Array.isArray(file.extractedTags) ? file.extractedTags.join(", ") : "-"}`);
        });
      } else {
        console.log("[SearchPage] No files fetched from backend.");
      }
    },
  });
          ...
          <div className="flex items-start gap-3">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <span>•</span>
                <span>by {message.user?.username || 'Unknown'}</span>
                <span>•</span>
