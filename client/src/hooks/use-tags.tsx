...
export function useEntityTagManager(entityType: string, entityId: string) {
  ...
  const handleTagsChange = async (newTags: Tag[]) => {
    ...
    // Find tags to add (only allow adding, not removing)
    const tagsToAdd = newTags.filter(tag => !currentTagIds.includes(tag.id));
    
    // Ignore removals - users can only add tags, not delete them
    const tagsToRemove: Tag[] = [];

    // Update local state to include new tags but keep existing ones
